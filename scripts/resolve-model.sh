...
# it reaches nightly.yml (tier read from the issue's Model Tier label) and
# claude.yml (tier read from a keyword in the triggering comment/issue text)
...
# a sentinel the caller checks for. In nightly.yml that means handing the
# issue back for a local session instead of running Claude in Actions;
# claude.yml doesn't offer the fable keyword at all, since a comment
# trigger is already a human asking for the run live.
#
# Used by .github/workflows/nightly.yml and .github/workflows/claude.yml.
set -eu
